"""Sequential WA1_D11 demo: right grasp, left grasp, then track.

This script keeps the existing trained grasp and tracking policies separate and
orchestrates them in one playback scene:

1. Run the right-hand grasp policy in the WA1_D11 manipulation scene.
2. Keep the right object attached, then run the left-hand grasp policy.
...
_DEFAULT_RIGHT_GRASP_CHECKPOINT = (
  "/home/robot706/yx/mjlab_v3/logs/rsl_rl/wa1_d11_grasp_cylinder/"
  "2026-04-01_19-30-13_left_arm_v1/model_3500.pt"
)
_DEFAULT_LEFT_GRASP_CHECKPOINT = (
  "/home/robot706/yx/mjlab_v3/logs/rsl_rl/wa1_d11_grasp_cylinder/"
  "2026-03-30_21-39-47_left_arm_v1/model_4999.pt"
)
_DEFAULT_TRACKING_CHECKPOINT = (
  "/home/robot706/yx/mjlab_v3/logs/rsl_rl/wa1_d11_tracking/"
  "2026-04-02_09-25-45_tracking_v1/model_3000.pt"
)
...
class GraspThenTrackPlayConfig:
  right_grasp_checkpoint_file: str = _DEFAULT_RIGHT_GRASP_CHECKPOINT
  left_grasp_checkpoint_file: str = _DEFAULT_LEFT_GRASP_CHECKPOINT
  ...
  right_grasp_stage: Literal["default", "match_checkpoint", "easy", "mid", "final"] = (
    "default"
  )
  left_grasp_stage: Literal["default", "match_checkpoint", "easy", "mid", "final"] = (
    "default"
  )
  grasp_success_hold_steps: int = 20  # 抓取成功连续帧数阈值
  grasp_success_max_site_distance_m: float = 0.10
  grasp_success_max_object_speed_mps: float = 1.0
  hard_switch_right_to_left: bool = True
  hold_right_arm_during_left_grasp: bool = True
  grasp_phase_speed: float = 0.2
  transition_phase_speed: float = 2.0
  right_to_left_transition_duration_s: float = 1.2
...
class SequentialGraspTrackPolicy:
  def __init__(
    ...
    grasp_success_max_site_distance_m: float,
    grasp_success_max_object_speed_mps: float,
    hard_switch_right_to_left: bool,
    hold_right_arm_during_left_grasp: bool,
    grasp_phase_speed: float,
    transition_phase_speed: float,
    right_to_left_transition_steps: int,
    ...
    self.env = env
    self.right_grasp_policy = right_grasp_policy
    self.left_grasp_policy = left_grasp_policy
    ...
    self.grasp_success_max_site_distance_m = max(grasp_success_max_site_distance_m, 0.01)
    self.grasp_success_max_object_speed_mps = max(grasp_success_max_object_speed_mps, 0.01)
    # Keep this attribute for backward-compatible CLI parsing, but the
    # right->left interpolation path is intentionally disabled.
    self.hard_switch_right_to_left = hard_switch_right_to_left
    self.hold_right_arm_during_left_grasp = hold_right_arm_during_left_grasp
    self.grasp_phase_speed = grasp_phase_speed
    self.transition_phase_speed = transition_phase_speed
    self.right_to_left_transition_steps = max(right_to_left_transition_steps, 1)
    ...
    # 状态机的状态 7个阶段：右手抓取、右到左过渡、左手抓取、过渡、跟踪前保持、跟踪、最终保持
    self.phase: Literal[
      "right_grasp",
      "right_to_left_transition",
      "left_grasp",
      "transition",
      "pre_track_hold",
      "tracking",
    ...
    self._right_to_left_transition_step = 0
    ...
    self._right_to_left_transition_start_joint = self.first_track_joint.clone()
    self._tracking_needs_frame0 = False
    self._right_object_attached = False
    self._left_object_attached = False
    self._left_side_hold_joint_target = self.default_joint_pos.clone()
    self._right_side_hold_joint_target = self.default_joint_pos.clone()

  def reset(self) -> None:
    env_ids = torch.arange(self.env.num_envs, device=self.env.device)
    self.phase = "right_grasp" # 一开始执行右手抓取
    self._grasp_success_counter = 0
    self._right_to_left_transition_step = 0
    self._transition_step = 0
    self._pre_track_hold_step = 0
  ...
  def __call__(self, _obs) -> torch.Tensor:
    ...
    if self.attach_object_during_tracking:
      if self._right_object_attached and self.phase in {
        "right_to_left_transition",
        "left_grasp",
        "transition",
        "pre_track_hold",
        ...
        self._attach_object_to_hand("right")
      if self._left_object_attached and self.phase in {
        "transition",
        "pre_track_hold",
        "tracking",
        "hold",
      }:
        self._attach_object_to_hand("left")
    # 判断执行哪个策略
    if self.phase == "right_grasp":
      return self._grasp_action("right")
    if self.phase == "right_to_left_transition":
      return self._right_to_left_transition_action()
    if self.phase == "left_grasp":
      return self._grasp_action("left")
  ...
  def _grasp_action(self, side: Literal["right", "left"]) -> torch.Tensor:
    ...
    if side == "right": # 调用策略
      grasp_action = self.right_grasp_policy(obs)
    else:
      grasp_action = self.left_grasp_policy(obs)
    track_action = grasp_action * self.grasp_to_track_ratio # 抓取动作转换为跟踪动作的比例
    if side == "right":
      # Right grasp only controls non-left-side joints.
      track_action = self._apply_side_hold(
        track_action, self.left_side_action_ids, self._left_side_hold_joint_target
      )
    elif self.hold_right_arm_during_left_grasp:
      # Left grasp only controls non-right-side joints.
      track_action = self._apply_side_hold(
        track_action, self.right_side_action_ids, self._right_side_hold_joint_target
      )

    if self._grasp_is_stable(side):
      self._grasp_success_counter += 1
    else:
      self._grasp_success_counter = 0

    if self._grasp_success_counter >= self.grasp_success_hold_steps: # 连续多帧满足抓取成功条件才进入下一个阶段
      if side == "right":
        self._right_side_hold_joint_target = self.robot.data.joint_pos[
          :, self.joint_ids
        ].clone()
        if self.attach_object_during_tracking:
          self._capture_object_attach_offset("right")
          self._right_object_attached = True
        # Disabled by request: skip right->left interpolation and hard-switch.
        # if self.hard_switch_right_to_left:
        #   self._begin_left_grasp()
        # else:
        #   self._begin_right_to_left_transition()
        self._begin_left_grasp()
      else:
        if self.attach_object_during_tracking:
          self._capture_object_attach_offset("left")
          self._left_object_attached = True
  ...
  def _right_to_left_transition_action(self) -> torch.Tensor:
    alpha = float(self._right_to_left_transition_step + 1) / float(
      self.right_to_left_transition_steps
    )
    target_joint = torch.lerp(
      self._right_to_left_transition_start_joint, self.default_joint_pos, alpha
    )
    action = self._raw_action_for_joint_target(target_joint)
    self._right_to_left_transition_step += 1
    if self._right_to_left_transition_step >= self.right_to_left_transition_steps:
      self._begin_left_grasp()
    return action
  ...
  def _grasp_is_stable(self, side: Literal["right", "left"]) -> bool:
    if side == "right":
      success = bool(torch.all(self.lift_command_right.compute_success()).item())
      height = float(self.cylinder_right.data.root_link_pos_w[0, 2].item())
      obj_pos = self.cylinder_right.data.root_link_pos_w
      obj_lin_vel = self.cylinder_right.data.root_link_lin_vel_w
      site_pos = self.robot.data.site_pos_w[:, self.right_grasp_site_id]
    else:
      success = bool(torch.all(self.lift_command_left.compute_success()).item())
      height = float(self.cylinder_left.data.root_link_pos_w[0, 2].item())
      obj_pos = self.cylinder_left.data.root_link_pos_w
      obj_lin_vel = self.cylinder_left.data.root_link_lin_vel_w
      site_pos = self.robot.data.site_pos_w[:, self.left_grasp_site_id]

    site_distance = torch.linalg.vector_norm(obj_pos - site_pos, dim=-1)
    object_speed = torch.linalg.vector_norm(obj_lin_vel, dim=-1)
    proximity_ok = bool(torch.all(site_distance <= self.grasp_success_max_site_distance_m).item()) # 物体与抓取点的距离是否在阈值内
    speed_ok = bool(torch.all(object_speed <= self.grasp_success_max_object_speed_mps).item()) # 物体的速度是否在阈值内
    # 抓取成功的条件：提升成功、物体高度、物体与抓取点的距离、物体速度
    return success and height >= (_CYLINDER_CENTER_Z + 0.10) and proximity_ok and speed_ok

  def _begin_right_to_left_transition(self) -> None:
    self.phase = "right_to_left_transition"
    self._right_to_left_transition_step = 0
    self._right_to_left_transition_start_joint = self.robot.data.joint_pos[
      :, self.joint_ids
    ].clone()
    self.motion_command.cfg.debug_vis = False
    self.motion_command.time_steps.zero_()
  ...
  def _begin_left_grasp(self) -> None:
    self.phase = "left_grasp"
    self._grasp_success_counter = 0
    self._right_side_hold_joint_target = self.robot.data.joint_pos[
      :, self.joint_ids
    ].clone()
  ...
  def _apply_side_hold(
    ...
    if action_ids.numel() == 0:
      return action
    frozen_action = action.clone()
    scales = self.track_action_scale[action_ids]
    nonzero = torch.abs(scales) > 1e-8
    if torch.any(nonzero):
      active_ids = action_ids[nonzero]
      active_scales = scales[nonzero]
      frozen_action[:, active_ids] = (
        hold_joint_target[:, active_ids]
        - self.default_joint_pos[:, active_ids]
      ) / active_scales
    return frozen_action

  def _capture_object_attach_offset(self, side: Literal["right", "left"]) -> None:
    if side == "right":
      site_pos = self.robot.data.site_pos_w[:, self.right_grasp_site_id]
      site_quat = self.robot.data.site_quat_w[:, self.right_grasp_site_id]
      obj_pos = self.cylinder_right.data.root_link_pos_w
      obj_quat = self.cylinder_right.data.root_link_quat_w
      self.right_object_pos_offset = quat_apply(quat_inv(site_quat), obj_pos - site_pos)
      self.right_object_quat_offset = quat_mul(quat_inv(site_quat), obj_quat)
      return

    site_pos = self.robot.data.site_pos_w[:, self.left_grasp_site_id]
    site_quat = self.robot.data.site_quat_w[:, self.left_grasp_site_id]
    obj_pos = self.cylinder_left.data.root_link_pos_w
    obj_quat = self.cylinder_left.data.root_link_quat_w
    self.left_object_pos_offset = quat_apply(quat_inv(site_quat), obj_pos - site_pos)
    self.left_object_quat_offset = quat_mul(quat_inv(site_quat), obj_quat)

  def _attach_object_to_hand(self, side: Literal["right", "left"]) -> None:
    if side == "right":
      site_pos = self.robot.data.site_pos_w[:, self.right_grasp_site_id]
      site_quat = self.robot.data.site_quat_w[:, self.right_grasp_site_id]
      obj_pos = site_pos + quat_apply(site_quat, self.right_object_pos_offset)
      obj_quat = quat_mul(site_quat, self.right_object_quat_offset)
      obj_pose = torch.cat([obj_pos, obj_quat], dim=-1)
      self.cylinder_right.write_root_link_pose_to_sim(obj_pose)
      self.cylinder_right.write_root_link_velocity_to_sim(self.zero_root_velocity)
      return

    site_pos = self.robot.data.site_pos_w[:, self.left_grasp_site_id]
    site_quat = self.robot.data.site_quat_w[:, self.left_grasp_site_id]
    obj_pos = site_pos + quat_apply(site_quat, self.left_object_pos_offset)
    obj_quat = quat_mul(site_quat, self.left_object_quat_offset)
    obj_pose = torch.cat([obj_pos, obj_quat], dim=-1)
    self.cylinder_left.write_root_link_pose_to_sim(obj_pose)
  ...
  def get_speed_multiplier(self) -> float:
    if self.phase in {"right_to_left_transition", "transition"}:
      return self.transition_phase_speed
    if self.phase in {"right_grasp", "left_grasp"}:
...
def _build_combo_env(
  motion_file: str,
  device: str,
  right_grasp_stage: Literal["easy", "mid", "final"],
  left_grasp_stage: Literal["easy", "mid", "final"],
  clip_actions: float | None,
  show_left_hand_cylinder: bool,
):
  combo_cfg = wa1_d11_grasp_cylinder_env_cfg(play=True) # 创建抓取环境
  ...
  apply_wa1_d11_grasp_cylinder_stage(
    combo_cfg, right_grasp_stage, command_name="lift_height_right"
  )
  apply_wa1_d11_grasp_cylinder_stage(
    combo_cfg, left_grasp_stage, command_name="lift_height_left"
  )

  env = ManagerBasedRlEnv(cfg=combo_cfg, device=device)
  wrapped_env = RslRlVecEnvWrapper(env, clip_actions=clip_actions)

  motion_cfg = wa1_d11_tracking_env_cfg(play=True).commands["motion"] # 创建跟踪任务的环境
  ...
  track_to_grasp_ratio = torch.zeros_like(track_scale)# 抓取动作转换为跟踪动作的比例
  ...
  grasp_obs_manager = ObservationManager({"actor": grasp_actor_cfg}, env) # 抓取环境和跟踪环境的观测不完全一样，所以创建两个ObservationManager，分别管理抓取和跟踪阶段的观测
...
def run_play(cfg: GraspThenTrackPlayConfig) -> None:
  ...
  right_grasp_checkpoint = Path(cfg.right_grasp_checkpoint_file).expanduser().resolve()
  left_grasp_checkpoint = Path(cfg.left_grasp_checkpoint_file).expanduser().resolve()
  ...
  if not right_grasp_checkpoint.exists():
    raise FileNotFoundError(
      f"Right grasp checkpoint not found: {right_grasp_checkpoint}"
    )
  if not left_grasp_checkpoint.exists():
    raise FileNotFoundError(
      f"Left grasp checkpoint not found: {left_grasp_checkpoint}"
    )
  ...
  # 加载策略
  right_grasp_policy, right_grasp_agent_cfg = _load_inference_policy(
    _GRASP_TASK_ID, right_grasp_checkpoint, device
  )
  left_grasp_policy, left_grasp_agent_cfg = _load_inference_policy(
    _GRASP_TASK_ID, left_grasp_checkpoint, device
  )
  ...
  # 确定grasper难度stage
  right_grasp_stage = _resolve_grasp_stage(
    cfg.right_grasp_stage, right_grasp_checkpoint
  )
  left_grasp_stage = _resolve_grasp_stage(
    cfg.left_grasp_stage, left_grasp_checkpoint
  )
  print(f"[INFO]: Using right grasp play stage: {right_grasp_stage}")
  print(f"[INFO]: Using left grasp play stage: {left_grasp_stage}")

  clip_actions = tracking_agent_cfg.clip_actions
  if right_grasp_agent_cfg.clip_actions is not None:
    if clip_actions is None:
      clip_actions = right_grasp_agent_cfg.clip_actions
    else:
      clip_actions = max(float(clip_actions), float(right_grasp_agent_cfg.clip_actions))
  if left_grasp_agent_cfg.clip_actions is not None:
    if clip_actions is None:
      clip_actions = left_grasp_agent_cfg.clip_actions
    else:
      clip_actions = max(float(clip_actions), float(left_grasp_agent_cfg.clip_actions))
  ...
  (
    ...
    grasp_to_track_ratio,
    track_action_scale,
  ) = _build_combo_env( # grasp env ≠ tracking env 所以创建一个新的环境
    motion_file,
    device,
    right_grasp_stage,
    left_grasp_stage,
  ...
  policy = SequentialGraspTrackPolicy(
    env=raw_env,
    right_grasp_policy=right_grasp_policy,
    left_grasp_policy=left_grasp_policy,
    ...
    grasp_success_max_site_distance_m=cfg.grasp_success_max_site_distance_m,
    grasp_success_max_object_speed_mps=cfg.grasp_success_max_object_speed_mps,
    hard_switch_right_to_left=cfg.hard_switch_right_to_left,
    hold_right_arm_during_left_grasp=cfg.hold_right_arm_during_left_grasp,
    grasp_phase_speed=cfg.grasp_phase_speed,
    transition_phase_speed=cfg.transition_phase_speed,
    right_to_left_transition_steps=int(
      round(cfg.right_to_left_transition_duration_s / raw_env.step_dt)
    ),
