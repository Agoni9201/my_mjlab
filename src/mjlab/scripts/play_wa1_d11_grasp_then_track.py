...
class GraspThenTrackPlayConfig:
  grasp_checkpoint_file: str
  tracking_checkpoint_file: str
  motion_file: str
  ...
  grasp_success_hold_steps: int = 20
  grasp_phase_speed: float = 0.2
  transition_phase_speed: float = 2.0
  transition_duration_s: float = 2.0
  track_start_delay_s: float = 0.3
  track_blend_duration_s: float = 0.4
  final_hold_duration_s: float = 1.5
  attach_object_during_tracking: bool = True
  show_left_hand_cylinder: bool = True
...
class SequentialGraspTrackPolicy:
  def __init__(
    ...
    self.object_pos_offset = torch.zeros(env.num_envs, 3, device=env.device)
    self.object_quat_offset = torch.zeros(env.num_envs, 4, device=env.device)
    self.object_quat_offset[:, 0] = 1.0
  ...
  def _grasp_is_stable(self) -> bool:
    success = bool(torch.all(self.lift_command.compute_success()).item())
    height = float(self.cylinder.data.root_link_pos_w[0, 2].item())
    return success and height >= (_CYLINDER_CENTER_Z + 0.10)

  def _begin_transition(self) -> None:
    self.phase = "transition"
    self._transition_step = 0
    self._transition_start_joint = self.robot.data.joint_pos[:, self.joint_ids].clone()
    self.lift_command.cfg.debug_vis = False
    self.lift_command.time_left.fill_(1e9)
    self.motion_command.cfg.debug_vis = False
    self.motion_command.time_steps.zero_()
    self.motion_command.time_left.fill_(1e9)
  ...
  def _capture_object_attach_offset(self) -> None:
    site_pos = self.robot.data.site_pos_w[:, self.grasp_site_id]
    site_quat = self.robot.data.site_quat_w[:, self.grasp_site_id]
    obj_pos = self.cylinder.data.root_link_pos_w
    obj_quat = self.cylinder.data.root_link_quat_w
    self.object_pos_offset = quat_apply(quat_inv(site_quat), obj_pos - site_pos)
    self.object_quat_offset = quat_mul(quat_inv(site_quat), obj_quat)

  def _attach_object_to_hand(self) -> None:
    site_pos = self.robot.data.site_pos_w[:, self.grasp_site_id]
    site_quat = self.robot.data.site_quat_w[:, self.grasp_site_id]
    obj_pos = site_pos + quat_apply(site_quat, self.object_pos_offset)
    obj_quat = quat_mul(site_quat, self.object_quat_offset)
    obj_pose = torch.cat([obj_pos, obj_quat], dim=-1)
    self.cylinder.write_root_link_pose_to_sim(obj_pose)
    self.cylinder.write_root_link_velocity_to_sim(self.zero_root_velocity)
...
def _build_combo_env(
  ...
  combo_cfg.commands["lift_height"].resampling_time_range = (1e9, 1e9)
  combo_cfg.episode_length_s = int(1e9)
  apply_wa1_d11_grasp_cylinder_stage(combo_cfg, grasp_stage)
