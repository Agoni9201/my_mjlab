...
class MotionLoader:
  def __init__(
    ...
    robot_joint_total: int | None = None,
  ) -> None:
    ...
    if joint_indexes is not None:
      if joint_pos.shape[1] == len(joint_indexes):
        pass
      elif robot_joint_total is not None and joint_pos.shape[1] == robot_joint_total:
        joint_pos = joint_pos[:, joint_indexes]
        joint_vel = joint_vel[:, joint_indexes]
      else:
        raise ValueError(
          "Motion joint dimension does not match the selected robot joints: "
          f"motion={joint_pos.shape[1]}, "
          f"selected={len(joint_indexes)}, "
          f"robot_total={robot_joint_total}."
        )
...
class MotionCommand(CommandTerm):
  ...
  def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRlEnv):
    ...
    self._joint_names = tuple(cfg.joint_names) or tuple(self.robot.joint_names)
    joint_indexes, _ = self.robot.find_joints(self._joint_names, preserve_order=True)
    self.joint_indexes = torch.tensor(
      joint_indexes, dtype=torch.long, device=self.device
    )
    self._joint_indexes_np = np.asarray(joint_indexes, dtype=np.int32)
    ...
    self.motion = MotionLoader(
      ...
      self.body_indexes,
      device=self.device,
      joint_indexes=self.joint_indexes,
      robot_joint_total=self.robot.num_joints,
    )
  ...
  def _write_reference_state_to_sim(
    ...
    """Clip joint positions and write reference state to sim."""
    soft_limits = self.robot.data.soft_joint_pos_limits[env_ids]
    joint_pos_env = torch.clip(
      joint_pos[env_ids],
      soft_limits[:, :, 0],
      soft_limits[:, :, 1],
    )
    joint_vel_env = joint_vel[env_ids]
    self.robot.write_joint_state_to_sim(joint_pos_env, joint_vel_env, env_ids=env_ids)

    if not self.robot.data.is_fixed_base:
  ...
  def _resample_command(self, env_ids: torch.Tensor):
    ...
    joint_pos[:, self.joint_indexes] = self.joint_pos
    joint_vel[:, self.joint_indexes] = self.joint_vel
    ...
    joint_pos_env = joint_pos[env_ids].clone()
    joint_pos_env[:, self.joint_indexes] += sample_uniform(
      lower=self.cfg.joint_position_range[0],
      upper=self.cfg.joint_position_range[1],
      size=(len(env_ids), len(self.joint_indexes)),
      device=joint_pos.device,  # type: ignore[arg-type]
    )
    soft_joint_pos_limits = self.robot.data.soft_joint_pos_limits[env_ids][
      :, self.joint_indexes
    ]
    joint_pos_env[:, self.joint_indexes] = torch.clip(
      joint_pos_env[:, self.joint_indexes],
  ...
  def _debug_vis_impl(self, visualizer: DebugVisualizer) -> None:
    ...
    if self.cfg.viz.mode == "ghost":
      ...
      for batch in env_indices:
        ...
        if free_joint_q_adr.size > 0:
          qpos[free_joint_q_adr[0:3]] = self.body_pos_w[batch, 0].cpu().numpy()
          qpos[free_joint_q_adr[3:7]] = self.body_quat_w[batch, 0].cpu().numpy()
        default_joint_pos = self.robot.data.default_joint_pos[0].cpu().numpy().copy()
        default_joint_pos[self._joint_indexes_np] = self.joint_pos[batch].cpu().numpy()
        qpos[joint_q_adr] = default_joint_pos
